...
import type WorldRegistry from "/data/worlds/worldregistry.ts";
...
import type { PlayerRegistry } from "/player/playerregistry.ts";
    ...
    return this.argumentFormats
      .map((format) => {
        ...
        if (
          ...
          let out: string;
          if (format.name) {
            out = `<${format.name}X> <${format.name}Y> <${format.name}Z>${
              format.type === "entityPosition"
                ? ` <${format.name}Yaw> <${format.name}Pitch>`
                : ""
            }`;
          } else {
            out = `<x> <y> <z>` +
              (format.type === "entityPosition" ? ` <yaw> <pitch>` : "");
          }
        ...
        if (["rangedNumber", "rangedInteger"].includes(format.type)) {
          ...
          return `${format.optional ? "[" : "<"}${
            rangedFormat.name ?? rangedFormat.type
          } (${rangedFormat.min}-${rangedFormat.max})${
            format.optional ? "]" : ">"
          }`;
        }
        if (format.optional) {
