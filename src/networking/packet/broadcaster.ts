import type { Packet, PacketIds } from "/networking/packet/packet.ts";
import type { Server } from "/networking/server.ts";
import type { Connection } from "/networking/connection.ts";
import type { PacketData } from "./packetdata.ts";
import { DestroySubscriptionManager } from "/utility/destroysubscriptionmanager.ts";
  ...
  public broadcast(data: Omit<T, "id">) {
    ...
    for (
      ...
      try {
        ...
        const newData = this.modifier ? this.modifier(data, connection) : data;
        promises.push(
