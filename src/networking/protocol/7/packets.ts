...
} from "/networking/packet/factories.ts";
import { STRING_OPTIONS } from "/networking/packet/packet.ts";
import { assertPacket } from "/networking/packet/utilities.ts";
import { StructuredParserBuilder as StructParserBuilder } from "/utility/datastruct.ts";
import type { Connection } from "/networking/connection.ts";
import Player from "/player/player.ts";
import Vector3 from "/datatypes/vector3.ts";
import { BlockIds } from "/data/blocks.ts";
...
import { sanitizeNetworkString } from "/utility/sanitizenetworkstring.ts";
import { crypto } from "@std/crypto";

const PROTOCOL_VERSION = 7;

export const identificationPacket7 = createBidirectionalPacket<
...
>({
  ...
  async receive(connection: Connection, data: Uint8Array) {
    ...
    const serverConfig = connection.serviceRegistry.get("config")?.server;
    const isNameVerificationEnabled = serverConfig?.data.server.verifyNames;
    const isLocalConnection = ["localhost", "127.0.0.1"].includes(
      connection.socket.remoteAddr.hostname,
    ) || connection.socket.remoteAddr.hostname.startsWith("192.168.");
    const isUnverifiedLocalNamesAllowed = serverConfig?.data.server
      .allowUnverifiedLocalNames;
    const isNameVerified = heartbeat?.salt &&
        sanitizeNetworkString(decoded.keyOrMotd).toLowerCase() ===
          encodeHex(
            await crypto.subtle.digest(
              "MD5",
              new TextEncoder().encode(
                heartbeat.salt + playerName,
              ),
            ),
          ) ||
      !heartbeat?.salt;
    ...
    clientPacket.send(connection, {
      ...
      name: "Prehistoric Classic",
      keyOrMotd: "A prehistoric classic server",
      userType: 0,
    }).catch(connection.onError.bind(connection));
    const worldRegistry = connection.serviceRegistry.get("worldRegistry");
...
export const levelInitializePacket7 = createSendablePacket<
...
export const levelDataChunkPacket7 = createSendablePacket<
...
export const levelFinalizePacket7 = createSendablePacket<
...
export const setBlockClientPacket7 = createReceivablePacket<
...
export const setBlockServerPacket7 = createSendablePacket<
