...
import type WorldRegistry from "./worldregistry.ts";
import type WorldParser from "./parsers/base.ts";
...
import type { DEFAULT_CONFIGS } from "/data/configs/constants.ts";
import type { Config } from "/data/config/config.ts";
...
export class World {
  ...
  async setBlock(position: Vector3, blockId: number): Promise<void> {
    ...
    for (const entity of this.entities.values()) {
      ...
      if (entity instanceof PlayerEntity) {
        ...
        const setBlockPacket = protocol.packets[PacketIds.SetBlockServer];
  ...
  async save(saveDir?: string): Promise<void> {
    ...
    const saveDirectory = saveDir ??
      this.serverConfig?.data.worlds.worldDir;
