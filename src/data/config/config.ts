...
import {
  ...
  handlers,
} from "/data/config/configfilehandlers.ts";
import { CONFIG_PATH } from "/data/configs/constants.ts";
...
export class Config<T extends ConfigData = ConfigData> {
  ...
  constructor({
    ...
    if (handler) {
      ...
      Deno.env.get("CONFIG_FORMAT") &&
      Deno.env.get("CONFIG_FORMAT") !== ""
    ) {
      const format = Deno.env.get("CONFIG_FORMAT")?.toLowerCase();
      if (format && format in handlers) {
  ...
  async save(): Promise<void> {
    ...
    await Deno.writeTextFile(this.getPath(), encoded);
  }
