...
*/
import type { Chatroom } from "/chat/chatroom.ts";
...
export class Player {
  ...
  public readonly emitter = new EventEmitter<PlayerEvents>();
  ...
  constructor(options: PlayerOptions) {
    ...
    this.serviceRegistry = options.serviceRegistry ||
      this.connection?.serviceRegistry;
      ...
      this.interceptors.chat["commands"] = {
        ...
        func: async (player, message) => {
          ...
          if (message.startsWith("/")) {
            const commandRegistry = this.serviceRegistry?.get(
              "commandRegistry",
            );
            if (!commandRegistry) {
              this.sendMessage(
